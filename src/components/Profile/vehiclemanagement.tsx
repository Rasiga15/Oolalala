...
import { useAuth } from '../../contexts/AuthContext';
import ConfirmationModal from '../common/ConfirmationModal';
import IdProofVerifyModal from '../common/IdProofVerifyModal';
...
const VehicleManagement: React.FC = () => {
  ...
  const checkIdProofVerification = async (): Promise<boolean> => {
    try {
      ...
      // Check if user has any document with verification status 'verified'
      const hasVerifiedDocument = documents.some(
        doc => doc.verificationStatus === 'verified'
      );
      
      console.log('Has verified document:', hasVerifiedDocument);
  ...
  const handleVerifyIdProof = () => {
    ...
    navigate('/id-proof'); // Navigate to ID proof verification page
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <IdProofVerifyModal
        ...
        onClose={() => setShowIdProofModal(false)}
        onVerifyIdProof={handleVerifyIdProof}
      />
    </div>
  );
};
