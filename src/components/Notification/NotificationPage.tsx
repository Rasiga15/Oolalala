...
import { useNavigate } from 'react-router-dom';
import { FiSearch, FiBell } from 'react-icons/fi';
import { FaCar, FaReceipt, FaCheckCircle, FaTimesCircle, FaStar } from 'react-icons/fa';
import roadTripPromo from '@/assets/NOTIFY.png';
import { 
  getNotifications, 
  Notification as ApiNotification,
...
const NotificationsPage: React.FC = () => {
  ...
  const handleRateTripClick = async (notification: Notification) => {
    // Mark all as read first
    if (!notification.read) {
      setIsMarkingAll(true);
    ...
    if (bookingId) {
      // Navigate to rating page with booking ID
      navigate('/trip-rating', { 
        state: { 
          bookingId: bookingId,
          source: 'notification',
      ...
      // If no booking ID, navigate to general rating page
      navigate('/trip-rating');
    }
  };

  const handleViewBookingClick = async (notification: Notification) => {
    // Mark all as read first
    if (!notification.read) {
      setIsMarkingAll(true);
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 mt-2 sm:mt-3">
                          ...
                          <div className="flex gap-2">
                            {/* Show Rate Trip button only for trip completed notifications */}
                            {/* {isTripCompleted && (
                              <button 
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleRateTripClick(notification);
                                }}
                                disabled={isMarkingAll || isLoadingBooking}
                                className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-full font-medium text-xs sm:text-sm transition-colors whitespace-nowrap flex items-center gap-1 disabled:opacity-70 ${
                                  !notification.read 
                             ...
                             */}
                            {/* Show View Booking button for notifications with booking ID */}
                            {hasBookingId && (
                              <button 
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleViewBookingClick(notification);
                                }}
                                disabled={isMarkingAll || isLoadingBooking}
                                className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-full font-medium text-xs sm:text-sm transition-colors whitespace-nowrap disabled:opacity-70 ${
                                  getButtonStyle(notification.type === 'ride_request' ? 'primary' : 'secondary')
                                }`}
                              >
                                {isLoadingBooking ? 'Loading...' : isMarkingAll ? 'Processing...' : 'View Booking'}
                              </button>
                            )}
                            
                            {/* For notifications without booking ID, show appropriate action button */}
                            {!hasBookingId && (
                              <button 
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleOtherNotificationNavigation(notification, null);
                                }}
                                disabled={isMarkingAll || isLoadingBooking}
                                className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-full font-medium text-xs sm:text-sm transition-colors whitespace-nowrap disabled:opacity-70 ${
